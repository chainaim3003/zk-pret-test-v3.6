...
export class EXIMNetworkHandler {
  ...
  async getEXIMNetworkMultiVerifierUtils(companyNames: string[]): Promise<any> {
    ...
    try {
      ...
      const existingContractAddress = 'B62qkEXIMContractAddressWillBeSetLater'; // Will be set in testnet.json
      let zkAppAddress: PublicKey;
      let zkApp: EXIMOptimMultiCompanySmartContract;

      // Set fee for transactions (needed for both existing and new contracts)
      const fee = this.getTransactionFee(currentEnvironment);

      if (existingContractAddress && currentEnv === 'TESTNET' && existingContractAddress !== 'B62qkEXIMContractAddressWillBeSetLater') {
        ...
        zkApp = new EXIMOptimMultiCompanySmartContract(zkAppAddress);
